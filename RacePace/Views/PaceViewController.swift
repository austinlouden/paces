    ...
    if constraints.isEmpty {
      constraints = [
        ...
        footer.bottomAnchor.constraint(
          equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: -kSpacing * 2),
      ]
...
extension PaceViewController {
  func setupTableView() {
    ...
    tableView.register(
      CustomDistanceCell.self, forCellReuseIdentifier: CustomDistanceCell.cellIdentifier)
  }
  ...
  func setupDataSource() {
    dataSource = UITableViewDiffableDataSource<Section, RaceResult>(
      tableView: tableView,
      cellProvider: { (tableView, indexPath, raceResult) -> UITableViewCell? in
        guard
          let cell = tableView.dequeueReusableCell(withIdentifier: PaceCell.cellIdentifier)
            as? PaceCell
        else {
  ...
  func updateUI(animated: Bool = true) {
    ...
    header.distanceLabel.text =
      controller.raceState.race == .custom
      ? controller.raceState.customRace?.distanceString() : controller.raceState.race.longString
  }
}
